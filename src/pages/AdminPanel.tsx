...
const AttendanceScanner = () => {
  ...
  const processToken = useCallback(async (token: string) => {
    ...
    setScanError('');
    try {
      const info = await mockApi.scanQrAttendance(trimmed);
      if (info.alreadyAttended) {
        setScanResult({ ...info, markedNow: false });
      } else {
        // Auto-mark attendance immediately on first scan
        await mockApi.markAttendance(
          info.teamId,
          info.memberRole as 'leader' | 'player' | 'substitute',
          info.memberRollNo,
        );
        setScanResult({ ...info, alreadyAttended: false, markedNow: true });
        toast({ title: '✅ Attendance Marked', description: `${info.memberRollNo} — ${info.teamName}` });
      }
    } catch (err: unknown) {
        ...
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className={`font-orbitron font-bold text-sm ${scanResult.alreadyAttended ? 'text-red-400' : 'text-green-400'}`}>
                  {scanResult.alreadyAttended ? 'ALREADY ATTENDED' : 'ENTRY ALLOWED'}
                </p>
                <p className="text-[10px] text-muted-foreground mt-0.5 uppercase tracking-widest">
                  {scanResult.alreadyAttended
                    ? 'This QR was already scanned — entry denied.'
                    : 'Attendance marked — welcome!'}
                </p>
              </div>
            </div>
